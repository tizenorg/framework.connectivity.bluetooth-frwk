...
tc_table_t tc_table[] =
{
	...
	{"bluetooth_gatt_remove_service", 99},
...
//			files[2] = "/opt/media/Images/image3.jpg";
		...
		case 95: {
			...
			char char_data[4] = {33, 44, 45, 66};
			int char_length = 4;
			char *char_flags[ ] = {"read", "write", "notify", "indicate"};
			int flags_length = 4;

			ret = bluetooth_gatt_add_characteristics(char_uuid,
				char_data,  char_length,
				char_flags, flags_length,
				svc_obj_path, &char_obj_path);
		...
		case 96: {
			char desc_value[4] = {11, 22, 33, 44};
			int value_length = 4;
			char *desc_uuid = g_strdup("000026ff-0000-1000-8000-00805f9b34fb");
			char *permissions = g_strdup("read");

			ret = bluetooth_gatt_add_descriptor(desc_uuid,
				desc_value, value_length,
				permissions, char_obj_path, &desc_obj_path);
		...
		case 99: {
			ret = bluetooth_gatt_remove_service(svc_obj_path);
