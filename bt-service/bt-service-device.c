...
bt_remote_dev_info_t *_bt_get_remote_device_info(char *address)
{
	...
	if (hash != NULL) {
		...
		dev_info->connected = value ? g_value_get_uchar(value) : BLUETOOTH_CONNECTED_LINK_NONE;
			...
			bluetooth_connected_link_t *connected)
{
	...
	value = g_hash_table_lookup(hash, "Connected");
	*connected = (bluetooth_connected_link_t)g_value_get_uchar(value);

	BT_DBG("connected link : %d", *connected);
