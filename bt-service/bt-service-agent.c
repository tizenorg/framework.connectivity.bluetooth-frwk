...
static gboolean __bt_is_major_class_computer(DBusGProxy *device)
{
	GHashTable *hash = NULL;
	GValue *value;
	GError *error = NULL;
	...
	BT_DBG("COD : 0x%X , MAJOR : 0x%X", device_class, major_class);

	if (major_class == BLUETOOTH_DEVICE_MAJOR_CLASS_COMPUTER)
		return TRUE;
