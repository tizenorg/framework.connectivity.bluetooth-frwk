...
static bluetooth_device_info_t *__bt_get_device_info_in_message(GVariant *parameters, int *ret)
{
	...
	g_variant_get(parameters, "(i&sun&sbbb@as)", &result, &address,
			&dev_class, &rssi, &name, &paired,
			&connected, &trust,  &string_var);
...
/*
			if (vconf_set_int(BT_OFF_DUE_TO_FLIGHT_MODE, 0) != 0)
				BT_ERR("Set vconf failed[BT_OFF_DUE_TO_FLIGHT_MODE]");
*/
			if (vconf_set_int(BT_OFF_DUE_TO_DEVICE_ERROR, 0) != 0)
				BT_ERR("Set vconf failed[BT_OFF_DUE_TO_DEVICE_ERROR]");

			if (vconf_set_int(BT_OFF_DUE_TO_TIMEOUT, 0) != 0)
				BT_ERR("Set vconf failed[BT_OFF_DUE_TO_TIMEOUT]");
		}
...
*/
		...
		g_free(device_info);
	}  else if (strcasecmp(signal_name, BT_DEVICE_ERROR) == 0) {
		BT_DBG("Device Error");
		if (disable_timer_id > 0)
			g_source_remove(disable_timer_id);
...
*/
		if (vconf_get_int(BT_OFF_DUE_TO_DEVICE_ERROR,
							&derr_value) != 0)
			BT_ERR("Fail to get the derr value");
		...
		g_free(device_info);
	}  else if(strcasecmp(member, BT_DEVICE_ERROR) == 0) {
		BT_DBG("Device Error");
		if (disable_timer_id > 0)
			g_source_remove(disable_timer_id);
