		...
		GArray **out_param1)
{
	...
	profile_proxy = dbus_g_proxy_new_for_name(g_conn, BT_BLUEZ_NAME,
				      device_path, BT_HANDSFREE_GATEWAY_INTERFACE);
	...
	func_data->req_id = request_id;
	if (!dbus_g_proxy_begin_call(profile_proxy, "Connect",
			(DBusGProxyCallNotify)__bt_hf_request_cb,
			func_data, NULL,
			G_TYPE_INVALID)) {
		BT_ERR("HFG connect Dbus Call Error");
		g_object_unref(profile_proxy);
		...
		GArray **out_param1)
{
	...
	if (!dbus_g_proxy_begin_call(profile_proxy, "Disconnect",
			...
			func_data, NULL,
			G_TYPE_INVALID)) {
		BT_ERR("HFG disconnect Dbus Call Error");
		g_object_unref(profile_proxy);
