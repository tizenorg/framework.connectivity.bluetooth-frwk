							...
							const char *agent_path)
{
	...
	case BT_AGENT_EVENT_FACTORY_RESET_REQUEST:
		g_strlcpy(event_str, "factory-reset-request", sizeof(event_str));
		break;
...
static gboolean __pincode_request(GapAgent *agent, DBusGProxy *device)
{
	...
	if (_bt_is_device_creating() == TRUE &&
		...
		gap_agent_reply_pin_code(agent, GAP_AGENT_ACCEPT, "0000", NULL);
	} else if (__bt_agent_is_hid_keyboard(device_class)) {
		BT_ERR("Passkey request is not supported");
		gap_agent_reply_pin_code(agent, GAP_AGENT_CANCEL, "", NULL);
	} else {
		BT_ERR("Pin request is not supported");
		gap_agent_reply_pin_code(agent, GAP_AGENT_CANCEL, "", NULL);
	}
								...
								guint passkey)
{
	...
	gboolean reset_is_required = FALSE;
	...
	if (major_class == BLUETOOTH_DEVICE_MAJOR_CLASS_AUDIO) {
		BT_INFO("Audio device. Launch passkey pop-up");
	...
	for (i = 0; i < no_of_device; i++) {
		...
		if (g_strcmp0(address, addr) == 0) {
			BT_INFO("This device is already in paired list");
			reset_is_required = FALSE;
			break;
		}

		if (info.device_class.major_class != BLUETOOTH_DEVICE_MAJOR_CLASS_AUDIO) {
			BT_INFO("New device");
			/* In tizen 2.3, not use this */
			/* reset_is_required = TRUE; */
			break;
		}
	}
	g_array_free(device_list, TRUE);

	if (reset_is_required) {
		BT_INFO("Launch factory reset pop-up");
		__launch_system_popup(BT_AGENT_EVENT_FACTORY_RESET_REQUEST, name,
...
#else
	BT_ERR("Accepting pairing request");
...
{
	BT_INFO("On Going Pairing is cancelled by remote");
							...
							const char *uuid)
{
	...
	if (!strcasecmp(uuid, A2DP_UUID)) {
		...
		if (ret) {
			BT_INFO("Already one A2DP device connected");
	...
	if (!strcasecmp(uuid, HFP_AUDIO_GATEWAY_UUID) ||
		...
		BT_INFO("Auto accept authorization for audio device (HFP, A2DP, AVRCP) [%s]", uuid);
	...
	if (!strcasecmp(uuid, NAP_UUID) ||
		...
		BT_INFO("Network connection request: %s", uuid);
...
fail:
	...
	DBG_SECURE("Authorization request for device [%s] Service:[%s]\n",
			address, uuid);
...
static gboolean __authorization_cancel_request(GapAgent *agent,
							const char *address)
{
	BT_INFO("On Going Authorization is cancelled by remote");
				...
				const gchar *address, const gchar *name)
{
	...
	if (__bt_agent_is_device_blacklist(lap_address, name)) {
		BT_INFO("Device is black listed");
		return FALSE;
	}
