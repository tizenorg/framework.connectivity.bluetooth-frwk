...
static gboolean is_advertising;
...
static int __bt_set_enabled(void)
{
	...
	if (vconf_set_int(BT_OFF_DUE_TO_DEVICE_ERROR, 0) != 0)
		BT_ERR("Set vconf failed\n");
...
gboolean _bt_is_connectable(void)
{
	...
	value = g_hash_table_lookup(hash, "Connectable");
	retv_if(value == NULL, BLUETOOTH_ERROR_INTERNAL);

	is_connectable = g_value_get_boolean(value);
	g_hash_table_destroy(hash);
...
int _bt_set_connectable(gboolean is_connectable)
{
	...
	if (vconf_get_bool(VCONFKEY_BT_DUT_MODE, &mode)) {
		BT_DBG("Get the DUT Mode fail");
		return BLUETOOTH_ERROR_INTERNAL;
	}

	if (mode != FALSE) {
		BT_DBG("Unable to set connectable in DUT Mode !!");
...
int _bt_set_advertising(gboolean enable)
{
	...
	if (mode != FALSE) {
		BT_DBG("Unable to start advertising in DUT Mode !!");
		return BLUETOOTH_ERROR_NOT_SUPPORT;
	}
	...
	if (is_advertising == TRUE && enable == TRUE)
		return BLUETOOTH_ERROR_IN_PROGRESS;
				...
				float interval_max, guint8 filter_policy)
{
	...
	if (mode != FALSE) {
		BT_DBG("Unable to set custom advertising in DUT Mode !!");
		return BLUETOOTH_ERROR_NOT_SUPPORT;
	}
	...
	if (interval_min > interval_max ||
			interval_min < BT_ADV_INTERVAL_MIN ||
			interval_max > BT_ADV_INTERVAL_MAX)
		return BLUETOOTH_ERROR_INVALID_PARAM;

	if (filter_policy > 0x03)
		return BLUETOOTH_ERROR_INVALID_PARAM;

	min = interval_min / BT_ADV_INTERVAL_SPLIT;
	max = interval_max / BT_ADV_INTERVAL_SPLIT;

	dbus_g_proxy_call(proxy, "SetAdvertisingParameters", &error,
			G_TYPE_UINT, min,
			G_TYPE_UINT, max,
			G_TYPE_UINT, filter_policy,
			G_TYPE_INVALID, G_TYPE_INVALID);
	...
	adv_params.interval_min = interval_min;
	adv_params.interval_max = interval_max;
	adv_params.filter_policy = filter_policy;
...
int _bt_set_advertising_data(bluetooth_advertising_data_t *adv, int length)
{
	...
	if (mode != FALSE) {
		BT_DBG("Unable to set advertising in DUT Mode !!");
		return BLUETOOTH_ERROR_NOT_SUPPORT;
	}
...
int _bt_set_scan_response_data(bluetooth_scan_resp_data_t *response, int length)
{
	...
	if (mode != FALSE) {
		BT_DBG("Unable to set scan response in DUT Mode !!");
		return BLUETOOTH_ERROR_NOT_SUPPORT;
	}
...
int _bt_add_white_list(bluetooth_device_address_t *device_address)
{
	...
	if (mode != FALSE) {
		BT_DBG("Unable to add white list in DUT Mode !!");
		return BLUETOOTH_ERROR_NOT_SUPPORT;
	}
...
int _bt_remove_white_list(bluetooth_device_address_t *device_address)
{
	...
	if (mode != FALSE) {
		BT_DBG("Unable to remove white list in DUT Mode !!");
		return BLUETOOTH_ERROR_NOT_SUPPORT;
	}
...
int _bt_clear_white_list(void)
{
	...
	if (mode != FALSE) {
		BT_DBG("Unable to clear white list in DUT Mode !!");
		return BLUETOOTH_ERROR_NOT_SUPPORT;
	}
